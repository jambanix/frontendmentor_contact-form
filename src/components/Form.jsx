...
export const Form = () => {
  ...
  return (
    <form noValidate onSubmit={handleSubmit(submit)} className="grid grid-cols-1 md:grid-cols-2 bg-white p-6 gap-4 rounded-md w-full max-w-[736px]">
      ...
      <Input error={errors.forename} {...register("forename")} label="First Name" className="md:col-start-1"/>
      <Input error={errors.surname} {...register("surname")} label="Last Name" className="md:col-start-2"/>
      <Input error={errors.email} {...register("email")} type="email" label="Email Address" className="md:col-span-2"/>
      ...
      <div className="flex flex-col col-start-1 md:col-span-2 gap-2">
        ...
        <div className="flex flex-col md:flex-row md:col-span-2 md:justify-between md:w-full gap-4">
          ...
          <ErrorMessage message={{...errors.enquiry}}/>
        </div>
      </div>

      <div className="flex flex-col gap-8 md:col-span-2">
        {/* Text area */}
        <TextArea error={errors.message} name="message" {...register("message")} label="Message" className="md:col-span-2"/>

        {/* Checkbox */}
        <Checkbox errors={errors} onSelect={handleAgreeButtonSelect} name="terms" label="I consent to being contacted by the team" {...register("terms")} className="md:col-span-2" />
